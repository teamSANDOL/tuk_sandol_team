...
현재 운영 중인 Gateway 서명 검증 + `X-User-ID` 전달 구조를 기준으로, MSA 통신 인증 규칙과 향후 JWKS 전환 계획을 함께 정리한다.
...
- 라우팅/관측/서명 검증과 헤더 전달을 담당
- 현재 운영 경계에서 인증 검증의 중심 역할을 수행
...
- 현행 운영은 Gateway 경계 검증 + `X-User-ID` 전달 체계를 사용한다.
- 각 MSA의 JWKS 직접 검증은 도입 전까지 필수 요건이 아니다.

```mermaid
flowchart LR
    C[호출 서비스] --> G[Gateway]
    G -->|X-User-ID 전달| M[대상 MSA]
    G -->|누락 헤더 기본 차단| R401[401]
    M --> V[컨텍스트/권한 검사]
    V --> P[비즈니스 처리]
    P --> OK[정상 처리]
    V -->|인증/컨텍스트 실패| R401
...
- Gateway 서명 검증 정책 통과 요청만 다운스트림으로 전달
