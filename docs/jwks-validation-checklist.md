# 필수 검증 체크리스트 (코드리뷰/릴리즈 게이트)

이 문서는 `docs/jwks-common-module-guideline.md` 요구사항을 PR 리뷰와 릴리즈 게이트에서 강제하기 위한 실행 체크리스트다.
...
- [ ] Gateway `X-Signature` 사용자 인증 의존 제거
- [ ] `X-User-Sub` 사용 지점 정리 또는 인증 경계 전용으로 제한
