...
6. 봇 서버는 이후 MSA 호출 시 `Authorization` + `X-User-ID`(= `sub`)로 호출
...
- MSA로 나가는 요청은 `Authorization` + `X-User-ID` 사용
- `X-User-ID` 값은 Keycloak `sub`를 그대로 사용
- Auth-Relay 내부 인증 경계에서는 `sub` 사용 가능
